...
if (iconMenu) {
	iconMenu.addEventListener("click", function (e) {
		...
		// swiperButtonPrev.classList.toggle('_active');
		// swiperButtonNext.classList.toggle('_active');
	});
}
...
// Яндекс карта --------------------------------------------------
ymaps.ready(init);  // ymaps - это пространство имён, через которые осушествляется доступ к компонентам карты.
...
function init() {      // Здесь мы создаём конструктор карты и привязываемся к нашему id="map"
	var map = new ymaps.Map('map', {
		center: [45.30, 34.16],      // Центр карты. Широта и долгота.    Питер [59.94, 30.32]
		zoom: 8,                     // Коэфициент масштабирования
		// controls: ['zoomControl'],   // Если эту строку убрать, то по умолчанию будет default и появится поисковая строка и другие элементы управления. Есл оставить массив пустым, то убирутся все эл. управления. Контроль масштаба лучше не убирать(понизит веб-доступность сайта). Нельзя убирать то что в левом нижнем и правом нижнем углу(это уголовно наказуемо). 
		behaviors: ['drag']          // Если эту строку убрать, то по умолчанию будет default и карту можно будет зумить
	});

	for (var i = 0; i < placemarks.length; i++) {
		geoObjects[i] = new ymaps.Placemark([placemarks[i].latitude, placemarks[i].longitude],
			{
				hintContent: placemarks[i].hintContent,                // Во внутрь хинт и балун можно добавить также вёрстку.
